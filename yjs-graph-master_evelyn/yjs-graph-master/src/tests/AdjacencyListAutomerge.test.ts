...
describe('AdjacencyListAutomerge', () => {
    let graph1: AdjacencyListAutomerge
    let graph2: AdjacencyListAutomerge

    beforeEach(() => {
        let amdoc1 = automerge.init<AdjacencyListAutomergeGraph>()
        amdoc1 = automerge.change(amdoc1, d => {
            d.map = {};
        });
        graph1 = new AdjacencyListAutomerge(amdoc1)

        let amdoc2 = automerge.init<AdjacencyListAutomergeGraph>()
        amdoc2 = automerge.merge(amdoc2, automerge.clone(amdoc1))
        graph2 = new AdjacencyListAutomerge(amdoc2)
    })

    it('should add a node from graph1 to both maps', () => {
        graph1.addNode('node1', 'node1', { x: 0, y: 0 } );
        AdjacencyListAutomerge.syncDefault([graph1, graph2]);
    ...
    it('should add a node from graph2 to both maps', () => {
    ...
    it('should add an edge from graph1 to both maps', () => {
        graph1.addNode('node1', 'node1', { x: 0, y: 0 });
        graph1.addNode('node2', 'node2', { x: 0, y: 0 });
    ...
    it('should add an edge from graph2 to both maps', () => {
        graph2.addNode('node1', 'node1', { x: 0, y: 0 } );
